...
def create_graph_visualization():
    ...
    with ManagedMORK.connect(url="http://127.0.0.1:8000") as server:
        with server.work_at("stress") as ws:
            ...
            # Get names - using double quotes like the demo script
            print("Fetching entity names...")
            names_result = ws.download("(name $id $name)", "($id $name)", max_results=10000)
            print(f"Names query result: {len(names_result.data) if names_result.data else 0} chars")
            print(f"Response status: {names_result.response.status_code if names_result.response else 'No response'}")
            ...
            output_file = "stress_knowledge_graph_fixed.png"
